...
var testFunc =  // onChange template 
...
console.log(' onchange fired for ask ', ask, ' inside my_script: ', script,' context: ',this);
       ...
       */
       ...
       let mydyn=askmatches['dyn_rest']={matched:null,complete:'fail'};// SAREBBE il qs da passare al next page ! E' STATUS A BREVE . se lo span e app allora usare state.appstatus
    ...
    // DO BL 
       ...
       // so qea will route to a  managed view as mod_wh does but the answere is not a item=xdata[matindex] fields but item.answere
           ...
           state.appstatus.dyn_match={dyn_rest:{match:mydata}};// ??
...
//            askmatches.dyn_rest={match:[]};
    ...
    // DO BL 
       ...
       state.appstatus.dyn_match={dyn_rest:{match:blResItem}};
...
5 voicename vgroup
 ...
 if(gr){
        ...
        let isStd;
        if(blRes==gr[4]) isStd=true;else false;
        ...
        mydyn.param.match=blResItem[1];//  name 
    ...
    /* old staff 
      ...
      mydyn.param.item={item:mydata[blRes]};// new way , use if to do a group view 
        ...
        // nb mydyn.match is the match on condition 
         ...
         if(state.appstatus&&state.appstatus.dyn_match&&state.appstatus.dyn_match.dyn_rest)
...
mustacheF.nmList=function (mapname,nmp,firstname){
...
//console.log('   ********** nmList called clVars is ',clVars );console.log('   ********** first name  is ',firstname );
   ...
   let ret;// error 
    if(mapname){// a notmatched model , look in excel if the model is descibed and get the notMatched prompt if there is 
            ...
            if(firstname=='.'||(firstname!='-')&&firstname==mapname)return ret;// first item
            else return ' e anche ' +ret;
     }
}
...
{{/beatles}}
    ...
    function listAitem(step,firstname){// a function factory, to render the item.name of a father context  askArray =[{name:'amodel/entity_name_testedas_$$_$%_inAsk'},,,] 
...
function listAitem1(firstname,col){// col is the row column index to display 
...
//return mustacheF.nmList(this,step,clVars)}// call an external function ( can be put in the same excel obj ? !)
if(firstname=='.'){firstname='-';fn='.';}else fn=firstname;
if(Array.isArray(this))el=this[col];else el=this;
return mustacheF.nmList(el,null,fn)}// will add e anche  dopo il primo el . call an external function ( can be put in the same excel obj ? !)
    ...
    televita:{// all var dyn added at containers values.excel/matches/askmatches of the convo room at defeult thread launch 
        ...
        dyn_medicine:{// used in  associazione a    :
            ...
            med_data:// will be used by onChange as db rows as array of string 
              ...
              11 when to 
              12 voicename
              13 get join with user med with status get/miss: 0/1
        */
                [
                    [11,'terace','redisdes','red RTCSessionDescription','data','piano 1','pesce','eggs backon gratis','vaial piano','prendi ascensore A presso la hall  ','08:00','10:00','ristorante terrazza',1],
                    [22,'hall','redisdes','red RTCSessionDescription','oggi branch gratis alle 11 ','piano terra','colazione all\'inglese','eggs backon gratis','vaial piano','recati presso la hall e prendi la sinistra','07:00','10:00','ristorante al piano terra',0],
                    [33,'giardino','redisdes','red RTCSessionDescription','data','piano 2','carne','veggs backon gratis','vaial piano','prendi ascensore B presso la hall','09:00','10:00','ristorante  in giardino',1]
                   ]
            ,
            data:// will be used by onChange as db rows as array of string if we try to manage a restaurant . all procedure to select are the same, changes only the selection data and answere view 
            /* row : 0 id
