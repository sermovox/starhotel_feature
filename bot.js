...
[3,'lavanderia','servizio 24 ore','calcei',1,'servizio di lavanderia','  quando è aperto e come arrivarci',' ristorante , portineria e taxi'],
    ...
    */
     ...
     */
        ...
        if(def){// def
            //mydyn.param.desired=['det_full'];
            mydyn.complete='queryagain';// todo
        }
...
// make sense to fill sel and def only if we got first row of cursor, other wise just fills mydyn.param.cursor={matches,patt,data} !
            ...
            mydyn.param.match=mydyn.param.vmatch=null;// nb different from a non dyn ask !!!
            //mydyn.param.cursor={rows:res.rows,resModel:{},data,param} ;// or use a arrays projection name and patt 
            mydyn.param.cursor={rows:res.rows,resModel:{}} ;// data useless, rows is enougth. resModel or use a arrays projection name and patt 
            /*
            resModel={val1:{
                patt:regexstr,
                vname:'pippo'
            ...
            res.rows.forEach(function(v,i){resModel[v[1]]={patt:v[14],vname:v[12]}});// calc matching [rows], then returns rows [] with just some cols (1:name)
// gr does not have meaning unless just a default or unique chars 
...
,
    ...
    televita:{// all var dyn added at containers values.excel/matches/askmatches of the convo room at defeult thread launch 
        ...
        */
                [
                    [11,'aspirina','aspirina','aspirina descr ','data','credenza 1','prima pasti ','se salti non riprenderla ','vai in credenza ',' scogliere pastiglia acqua e bere  ','prima pasti ','10:00','aspirina','col',,,,,,,,true],
                    ...
                    [33,'prostamol','prost*','prostamol descr','data','credenza 1','prima pasti ','se salti non riprenderla','vai in credenza',' scogliere pastiglia acqua e bere ','prima pasti ','10:00','prostamol','rest',,,,,,,,true],
                   ]
            ,
            data:// will be used by onChange as db rows as array of string if we try to manage a restaurant . all procedure to select are the same, changes only the selection data and answere view 
            /* row : 0 id
...
*/
[
[0,'col','il tuo programma prevede di assumere le seguenti pastiglie ',' avverti l operatore se hai problemi collaterali, ultimamente l aspirina è da preferire sciolta prima di ingiarla. ',1,' pastiglie ','  quando è aperto e come arrivarci',' ricorda anche che devi fare dei prelievi di urine'],
[1,'rest','master shef cucina internazionale','si prega prenotare per menu pesce ',1,'ristorante','  quando è aperto e come arrivarci',' ristorante , portineria e taxi'],
[2,'portineria','full service','calcei',1,'portineria','  quando è aperto e come arrivarci',' ristorante , portineria e taxi'],
[3,'lavanderia','servizio 24 ore','calcei',1,'servizio di lavanderia','  quando è aperto e come arrivarci',' ristorante , portineria e taxi'],
]
