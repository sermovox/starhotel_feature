...
var dyn_star_booking_desk =  // used in vita , ...
...
*/
  ...
  /* TODO 
    try aso to see if there is some match in res trying to match some user text to see if he specified the resource rest/col and also a specific selection 

    ex: test vars.ask_prendere_medicine agenst the patt
    cond(2,vars.ask_prendere_medicine,cQ);
...
var dyn_star_f_desk =  // used in vita , ...
...
*/ 
    ...
    if(matches.mod_assumere_med){// depending from user request give different info 
       let mat=matches.mod_assumere_med.match;
    if(mat=='contr'||mat=='no'){// so the user need is to to check or anyway to prompt the queryed list (contr= controllare lista )
       // mydyn.param.desired='miss';// ['when']// the prpoperty more important to give: when  something is somewhat
        // mydyn.param.when=mydata[blRes][8];// e/o mydyn.param.wh=mydata[blRes][8];
...
console.log(' onchange fired for ask ', ask, ' inside my_script: ', script,' context: ',this);
   ...
   function runQuery(mydata,cq){// find rows in mydata matching/whering columns/value as specified by cq ( can also be used the local cQ without use a param !)
    /*   cQ={cval:[],ccol:[]};// init query where clauses , cval =values     ccol= column to where 
        esempio :
          cQ={cval:[val7obj,val9obj],ccol:[7,9]}    
            >> si cercano le  riga di mydata in cui in col/index 7 c'e un valore == val7obj  : mydata[x][7]==val7obj
...
let fwAskOnChange={// ={modelname:{askname1:functionref1,,,},,}. usually model is the cmd name .  functions : in will be injected on models.onchange
                    ...
                    star_desk :{ dyn_medicine:dyn_star_f_desk,ask_afterpilldet:dyn_star_booking_desk}
...
module.exports ={init,onChange:fwAskOnChange,buildF,getappWrap,mustacheF,modsOnAsk,vfwF};// onChange:will overwrite directive onchange,getappWrap will now mng session recovery
                // .onChange=fwAskOnChange={cmd:{askname1:onchangefunc1,,,,,},,,}
