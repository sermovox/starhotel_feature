...
class Botkit {
    ...
    constructor(config) {
        ...
        const dialogState = this.conversationState.createProperty(this.getConfig('dialogStateProperty'));//  dialogStateProperty= 'dialogState'
    ...
    usePlugin(plugin_or_function) {
        ...
        if (typeof (plugin_or_function) === 'function') {
            plugin = plugin_or_function(this);// factory
        }
        else {
    ...
    configureWebhookEndpoint() {
        if (this.webserver) {
            ...
            this.webserver.get('/pippo', (req, res) => {
                // Allow the Botbuilder middleware to fire.
                // this middleware is responsible for turning the incoming payload into a BotBuilder Activity
                // which we can then use to turn into a BotkitMessage
                res.send('ciao');
            });
            this.webserver.get('/echo', (req, res) => {
                let echo='void';
                if(req.query.echo)echo=req.query.echo;
                res.send(echo);
            });
        }
    ...
    handleTurn(turnContext) {
        return __awaiter(this, void 0, void 0, function* () {
           ...
           console.log('\n\n >>>>  INCOMING ACTIVITY: text: ', turnContext.activity.text);//turnContext.activity);
                ...
                this.middleware.ingest.run(bot, message, (err, bot, message) => __awaiter(this, void 0, void 0, function* () {
                    if (err) {
                        ...
                        if (interrupt_results === false) {
                            ...
                            // botbuilder-dialogs/lib/dialogcontext 
    ...
    saveState(bot) {
        return __awaiter(this, void 0, void 0, function* () {
            yield this.conversationState.saveChanges(bot.getConfig('context'));// bot.getConfig('context')=tc
        });
    }
    /**
