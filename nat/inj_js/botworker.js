...
class BotWorker {
    ...
    */
    ...
    replaceDialog(id, options) {
        return __awaiter(this, void 0, void 0, function* () {
            if (this._config.dialogContext) {
                console.log(' replace dialog in bot , options: ',options);
                var optionadd=Object.assign({ user: this.getConfig('context').activity.from.id, channel: this.getConfig('context').activity.conversation.id }, options);
                console.log(' replace dialog in bot , optionadd: ',optionadd);
                yield this._config.dialogContext.replaceDialog(id + ':botkit-wrapper', optionadd);
                // make sure we save the state change caused by the dialog.
                // this may also get saved again at end of turn
                yield this._controller.saveState(this);
            }
