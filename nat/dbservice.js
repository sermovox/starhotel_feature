    ...
    */
    ...
    async function 
       ...
       // old                  (req, isDb_Aiax, schema_url, sess_clone, sess_dyn, paramList, prefill, map, afterallDyncalc) // a inner of main ctl code, req has different  std ctl chain format !
                        ...
                        (conn,entSchema,text_,wheres,idspace,isDb_Aiax)// also entity ??, temporarely set schemaname = entityname
                        // last change conn added in param as db connector 
 ...
 let wm;// the  master where clouse map ( x no join where ). 
   ...
   prom=new Promise(function(resolve,reject){resolve_=resolve;reject_=reject});// creates a promise that launch a function that wiil garantee tha sometime the promise resolve and reject cbs will be called (as middleware cb return resolve will be called )
            ...
            if (isDb_Aiax) {// 072020 db :    call db api from this db query  adapter  
                ...
                if (myschemaname1) mymodel = conn.model(myschemaname1);
...
let res={// called by  exsec  , will call resolve_
    ...
    results:function(  bytes,code ){// used 
        ...
        if(code==0){console.log('db query exit code ',code,' returning {reason:runned,rows: ',bytes,'}');
            resolve_({rows:bytes,reason:'runned'});
         }
        else {
            console.error('db query exit code ',code,' returning {reason:runned,rows: ',bytes,'}');
            resolve_({rows:bytes,reason:'err'});// rows can be array?
        }
    }
...
// internal custom REST data service . it will use a db servico mapping entity with db and collection 
...
// cnnection manager :
// std internal db with 2 rel :
// old : var schemaset1=[];schemaset1.push(['Master',BookSchema]),schemaset1.push(['J_1_m',J_1_m]);schemaset1.push(['J_n_m',J_n_m]);
//schemasets:{},
// schemasets[restaurant]schemaset1;// one item x a db to connect ( all schema data x the db ) the key can be another but in getconn we must add that 
...
/* >>>  will create/attach a db on server , then set the schema on collections and return the connector singlethon conns[db] :
 	singlethon is a connection ( create the table if not exixst ) to a db table with schema models as descbibed in :
		schemasets=	{ 	dbname1:[
						 ['collection1',schemainstance1],// table/collection 1 name and instance
						 ['collection2',schemainstance2],,,
				 		]
					dbname2:[],,,
				}


var schemarest=[];schemarest.push([Master__,RistorantiBLSchema]);
if(J_1_m__)schemarest.push([J_1_m__,J_1_m]);
if(J_n_m__)schemarest.push([J_n_m__,J_n_m]);
// set those :
// to set : schemarest.push(['J_1_m',null]);schemaset1.push(['J_n_m',null]);
*/
...
(node:17514) [DEP0018] DeprecationWarning: Unhandled promise rejections are deprecated. In the future, promise rejections that are not handled will terminate the Node.js process with a non-zero exit code.
       ...
       /*   conn.on('reconnected', () => { console.log('Connection Reestablished') });          
		...
		schemasets[db].forEach(function (item, index) {
   				//conn.model(schemasets[db][0],schemasets[db][1]);
   				conn.model(item[0],item[1]);
			});*/
			//conn.model('Master', BookSchema);
