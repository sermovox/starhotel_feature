...
let init = function () {// register bank (dynJs) function onChange x script/dynfield-key bound to dynJs[myscript]
    ...
    // NB NB all ask using framework ( $$ , useronchange ) must be registered with initCmd() , otherwire we get error askmatches not found in conversation.js
...
function initCmd(myscript_, usrAppSt, monchange, mod_dir) {//
    ...
    */
    ...
    let model = mod_dir || myscript_;// the name of cmd in onchange.js dynJs

    if (model) {
        if (dynJs[model]) directive = dynJs[model]; else {
            // no directive x this cmd so minimum support
            if(!monchange||monchange.length==0){
                nodirective=true;
            }
            else{
                console.log('\n starting FW initCmd ,cant find any models or directives for cmd   ', myscript_);return;
