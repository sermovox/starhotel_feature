...
if(result){
    ...
    for (let ada in result) {
        let sname=result[ada].name;
...
async function start(dateFrom_, inter) {// dateFrom "2016-02-09T00:00:00-08:00"  is iso from duck with time shift (-8 ), will be tranf to ROME local time 
   ...
   let ctl=qq.ctl;// get from a model reference (query also in different entity )the ctl part
    ...
    let serviceId = eventId = ctl.serviceId, performerId = ctl.perfId, unitId = '2', qty = 1;
        ...
        if (datas) {//  availableTime{  success:true,
            ...
            for (let ada in datas) {// datas={ada='2020-12-30':["09:00:00","10:00:00","11:00:00",,,],,,,,}
                ...
                datas[ada].forEach(tim => {// tim="09:00:00"
                   ...
                   let selItem={// query.cursor.resModel[stdT] = {// will point to rows[index].value
    ...
    */
    let inter = 4;// interval
        ...
        if (qq) {// a model is alredy filled so we have a ctl status 
            ...
            if (selStat == -8) {// the ctl has alredy set the event structure qq.ctl., and choose the performer 
        ...
        */
        ...
        async function getEvent() {// start here the ctl session if no ctl status structure in qq=qs.curStatus
        ...
        async function setPerformerSel() {// after coming back with a selected service , build the performer selector with the same status  query.ctl.eventSt
            selStat = -8;// initial status on the fsm recover a event (service/performer)  structure 

            // so as done x query model book_res_child , filled by a matcher with url refearring to this ctl  
            qq = getPerfs();// build ctl structure , fill simple query model 
