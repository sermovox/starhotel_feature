...
console.log('***** instantiating Botkit CMS');
    ...
    init(botkit,dsIndex) {
        ...
        if(dsIndex)this.ds=this._controller.dialogSets[dsIndex];
            else this.ds=this._controller.dialogSet;
...
console.log('*** downloading script/thread ', thread.topic);
                ...
                let d = new botkit_1.BotkitConversation(script.command, this._controller,this.ds);// create cmd obj//let d = new botkit_1.BotkitConversation(script.command, this._controller);// create cmd obj
                d.script = threads;// add threads obj
                botkit.addDialog(d,this.ds);//botkit.addDialog(d);// add dialog/cmd to dialog set 
            });
        });
    }
    /**
    ...
    before(script_name, thread_name, handler) {
        let dialog = ds.find(script_name);//let dialog = this._controller.dialogSet.find(script_name);
        if (dialog) {
    ...
    */
    onChange(script_name, variable_name, handler) {
        let dialog = ds.find(script_name);//let dialog = this._controller.dialogSet.find(script_name);let dialog = this._controller.dialogSet.find(script_name);
        if (dialog) {
