...
function change2Child(msgs){// added 082019, see howtoinsertagotoachildMess in documents/prove
    ...
    //    console.log('*** cms change2Child search  a child to fire in ask  : ',msg.collect.key);
            msg.collect.options.forEach((opn) => {
                ...
                if(opn.action&&opn.action=='execute_script'){//the child dialog
                   ...
                   if(opn.execute&&opn.execute.script&&opn.execute.script.charAt(0)=='_'){
                    ...
                    console.log('\n *** cms change2Child found a child to fire to : ',opn.execute.script);
...
class BotkitCMSHelper {
    constructor(config) {
         ...
         * Botkit Plugin name
         */
console.log('***** instantiating Botkit CMS NBNB : this file cms.js is in module botkit-plugin-cms \n  not botkit module !!!!!!!!!');
...
console.log('*** Dialogs loaded from Botkit CMS');
    ...
    apiRequest(uri, params = {}, method = 'GET') {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            console.error('Make request to Botkit CMS: ', req);// luigi debug only
            return new Promise((resolve, reject) => {
                request(req, function (err, res, body) {
                    if (err) {
                        debug('Error in Botkit CMS api: ', err);
                        console.error('Error in Botkit CMS api: ', err);// luigi debug only
                        return reject(err);
                    }
                    else {
                        debug('Raw results from Botkit CMS: ', body);
                        console.error('Raw results from Botkit CMS: ', body);// luigi debug only
                        let json = null;
                        try {
...
console.log('*** downloading script/thread ', thread.topic);
    ...
    testTrigger(bot, message) {
        return __awaiter(this, void 0, void 0, function* () {
           ...
           if (command.command) {
            console.log('cms matched trigger: ',message.text,' , for cmd  ',command.command);
                //return yield bot.beginDialog(command.command);// without passing message.text ??    return yield bot.beginDialog(command.command,{thread:'some'});
                // set as starting cmd option start_ctx. can be found as vars.start_ctx
                return yield bot.beginDialog(command.command,{start_ctx:message.start_ctx});// without passing message.text ??    return yield bot.beginDialog(command.command,{thread:'some'});
