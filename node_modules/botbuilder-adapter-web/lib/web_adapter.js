...
const clients = {};
const fs = require('fs');
const logchData=true,logtext=true;

// luigi 032020
const logger=function(message,ch,send){// a duplicate input logger, should be get as vcontroller plugins vcontroller.vfw.logger like in conversation.js
    if(!message.text||!logtext)return;
    let x,mylog='\n',ldata;
        if(send){x='bot send key template: '+message.key+' ';}else{ x=' bot receive from '+message.user;mylog+='\n'}
        if(logchData)ldata=tracedata(message);
    mylog+=new Date().toUTCString()+' web adapter channel '+ch+x+', text : '+message.text.replace(/(\r\n|\n|\r|\s+)/gm," ").substring(0,300);
    mylog+=' ... ';
    if(ldata)mylog+=ldata;
    fs.appendFile('bot.log', mylog, function (err) {
        if (err) return console.log(err);
      //console.log('Appended!');
     });
     function tracedata(msg){
         let ret='';
         for(let e in msg){if(e!='text')ret+=e+': ' + msg[e]+'; ';}
         return ret;
     }
...
class WebAdapter extends botbuilder_1.BotAdapter {
    ...
    init(botkit) {
        ...
        botkit.ready(() => {
            if(botkit.http)this.createSocketServer(botkit.http, this.socketServerOptions, botkit.handleTurn.bind(botkit));
...
console.log(' Web Adapter,  prova');
    ...
    /**
     * Bind a websocket listener to an existing webserver object.  >>> no webserver but http server , luigi 122020
     ...
     */
    createSocketServer(server, socketOptions = {}, logic) {// socketOptions = {path:'/x'},// 
        let ds;if(socketOptions.path)ds=socketOptions.path.substring(1);// added luigi 122020
        this.wss = new WebSocket.Server(Object.assign({ server }, socketOptions));// on defined uri !!! here server= controller.http 
        function heartbeat() {
            this.isAlive = true;
        }
...
console.log(' Web Adapter,  receiving '+payload);
                    ...
                    // add ds , so controller lauch the dialogset associated with ds :(future use in branch multiDS)
                    if(ds)activity.channelData.ds=ds;
    ...
    sendActivities(context, activities) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            for (var a = 0; a < activities.length; a++) {
               ...
               // let text=message.text;if(text)text=text.replace(/(<.*?>)|(/n)/gs,'');//Regex.Replace(mesage.text,"<.*?>",string.Empty);
    ...
    processActivity(req, res, logic) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (context.turnState.get('httpBody')) {
                res.json(context.turnState.get('httpBody'));// can be 2 or more messages !!!
            }
            else {
                res.end();
            }
