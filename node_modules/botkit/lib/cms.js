            ...
            msg.collect.options.forEach((opn) => {
                ...
                if(opn.action&&opn.action=='execute_script'){//the child dialog
                   ...
                   if(opn.execute&&opn.execute.script&&opn.execute.script.charAt(0)=='_'){
                    ...
                    console.log('*** cms change2Child found a child to fire to : ',opn.execute.script);
...
class BotkitCMSHelper {
    constructor(config) {
         ...
         * Botkit Plugin name
         */
console.log('*****x instantiating Botkit CMS');
            ...
            return new Promise((resolve, reject) => {
                request(req, function (err, res, body) {
                    if (err) {
                        ...
                        console.error('Error in Botkit CMS api: ', err);// debug only
                        return reject(err);
                    }
                    else {
                        debug('Raw results from Botkit CMS: ', body);
                        console.error('Raw results from Botkit CMS: ', body);// debugonly
                        let json = null;
                        try {
