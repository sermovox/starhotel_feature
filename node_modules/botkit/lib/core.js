...
let logs;//=function(){};// can be reset by bot.js using : controller.logs=vctl.vFw.logs;
/**
...
class Botkit {
     ...
     */
    constructor(config) {
        this.logs=logs;// will be reset by bot.js after
        /**
        ...
        if (this._config.disable_webserver !== true) {
            if (!this._config.webserver) {
                ...
                this.webserver.use('/x', express.static('/home/luigi/fs/home/luigi/localdev/starhotel_feature'+  '/public'));// dont work
                ...
                this.http.listen(this._config.port || 3000, () => {
                    if (this._config.disable_console !== true) {
                        console.log(`Webhook endpoint online:  http://localhost:${this._config.port || 3000}${this._config.webhook_uri}`);
                    }
        ...
        if (!this._config.adapter) {
            ...
            if (this.webserver) {
                if (this._config.disable_console !== true) {
                    console.log(`Open this bot in Bot Framework Emulator: bfemulator://livechat.open?botUrl=` + encodeURIComponent(`http://localhost:${this._config.port || 3000}${this._config.webhook_uri}`));
                }
            }
        }
        else {
        ...
        if (this.adapter) {
            ...
            this.usePlugin(this.adapter);// .usePlugin(adapter={name:pname,,})plugin name pname: when controller ready call : adapter.init(controller) to set websocket ......
        }
    ...
    configureWebhookEndpoint() {
        if (this.webserver) {
            ...
            // test temporely the sub uri added to load in future a ds customized. the browser will subdomail.sermovox.com/dsname
            // so the download page must be anyway subdomail.sermovox.com/index.html but the ws will be requested on uri subdomail.sermovox.com/dsname
            // https://expressjs.com/it/starter/static-files.html

            // for x dialogset downloaded from specific cms endpoint , we should download anyway the def page. add static middleware x sub uri /x 
            this.webserver.use('/x', express.static( '/public')); // o app.use('/static', express.static(__dirname + '/public'));
     ...
     */
    handleTurn(turnContext) {// è promise : quando si risolve (risoluzione di continueDialog() o ingest() )il adapter manda la risposta 
        return __awaiter(this, void 0, void 0, function* () {
            debug('INCOMING ACTIVITY:', turnContext.activity);
            let myout='\n===============================================================\nconvId:'+turnContext.activity.conversation.id+'\n >>>>  INCOMING ACTIVITY: text: '+ turnContext.activity.text+'\n';
           this.logs(myout);//turnContext.activity);
           console.log(myout);
            // Create a dialog context
            const dialogContext = yield this.dialogSet.createContext(turnContext);// incoming turns get status from dialogstate service 
            // Spawn a bot worker with the dialogContext
            const bot = yield this.spawn(dialogContext);
            // reset current stack 
            let matchcmd,cmd;
            //if(textFwCmd&&turnContext.activity.text&&(matchcmd=turnContext.activity.text.match(/(?:^|\s)%%(.+)-.*;(?:\s|$)/i)))cmd=matchcmd[1]; let wr;
            //if (cmd=='resetDial'){
                let wr;
                if(textFwCmd&&turnContext.activity.text&&turnContext.activity.text.indexOf('%%resetDial-')>=0){
                wr= yield bot.cancelAllDialogs();// text=...%%resetDial-ok;......
                console.log('convo.handleTurn  received a %%resetDial-; so cleared the dialog stack');}// then goon with matching text from dialog reset
     ...
     */
    ingest(bot, message) {// **** ingest(): run handler for the message event message.type . hears handlers are run using listenForTriggers(), a special event handler that choose the handler that is triggered
        return __awaiter(this, void 0, void 0, function* () {
            return new Promise((resolve, reject) => __awaiter(this, void 0, void 0, function* () {
                const listen_results = yield this.listenForTriggers(bot, message);
                //                                  listenForTriggers : will fire a special handler for the message.type event
                //                                 ****** it will run the handler of the triggered registered hears : triggers[t].handler.call(this, bot, message);
                if (listen_results !== false) {// hears handler found for some event(=message.type) fired by the message
                    ...
                    this.middleware.receive.run(bot, message, (err, bot, message) => __awaiter(this, void 0, void 0, function* () {
                        ...
                        // ***** Trigger event handlers registered with .on , will run : this._events[event][h].call(bot, bot, message);
     ...
     *  > * Evaluates an incoming message EVENTS for triggers created with `controller.hears()` and fires any relevant handler functions.
     * @param bot {BotWorker} An instance of the bot
     * @param message {BotkitMessage} an incoming message
     */
            ...
            if (this._interrupts[message.type]) {
                ...
                for (var t = 0; t < triggers.length; t++) {
                    ...
                    if (test_results) {
                        ...
                        const trigger_results = yield triggers[t].handler.call(this, bot, message);
                        return trigger_results;// false or true (will be run also waiting hears dialog in stack !)
                    }
                }
                // nothing has triggered...return false
                return false;
     ...
     * 
     *  >>>>> register trigger = {pattern: pattern,handler: handler,type: null}; on event queue
     * 
     */
    ...
    spawn(config) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            else if (config instanceof botbuilder_dialogs_1.DialogContext) {// preferred
                config = {
