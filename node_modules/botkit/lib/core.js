...
class Botkit {
    ...
    constructor(config) {
        ...
        max3::
        this.dialogSets=[];// additional group of customers max 3 
        this.dialogStates=[];// additional group of customers max 3 
    ...
    registerDSet(index) {
        this.conversationState = new conversationState_1.BotkitConversationState(this.storage);
        let  dialogState = this.conversationState.createProperty(this.getConfig('dialogStateProperty'+index));//  dialogStateProperty= 'dialogState'
        this.dialogSets[index] = new botbuilder_dialogs_1.DialogSet(dialogState);
        // useless this.dialogState[index] = dialogState;
    ...
    handleTurn(turnContext) {
        return __awaiter(this, void 0, void 0, function* () {
           ...
           let dsIndex=0,dsI=this.dialogSet;
           if(turnContext.activity.channelData.ds){dsIndex=turnContext.activity.channelData.ds;
           if(dsIndex>0&&dialogSets[sdIndex])dsI=this.dialogSets[sdIndex];}
            // Create a dialog context
            const dialogContext = yield dsI.createContext(turnContext);
    ...
    spawn(config,ds) {
        return __awaiter(this, void 0, void 0, function* () {
            if (config instanceof botbuilder_1.TurnContext) {
                config = {
                    dialogContext: yield ds,//dialogContext: yield this.dialogSet.createContext(config),
                    context: config,
            ...
            if (this.adapter.botkit_worker) {
                ...
                worker = new botworker_1.BotWorker(this, config,ds);
            }
            return new Promise((resolve, reject) => {
     ...
     */
    addDialog(dialog,ds_) {
        // add the actual dialog
        let ds;
        if(ds_)ds=ds;else ds=this.dialogSet;
        ds.add(dialog);
        // add a wrapper dialog that will be called by bot.beginDialog
        // and is responsible for capturing the parent results
        ds.add(new botbuilder_dialogs_1.WaterfallDialog(dialog.id + ':botkit-wrapper', [//this.dialogSet.add(new botbuilder_dialogs_1.WaterfallDialog(dialog.id + ':botkit-wrapper', [
            (step) => __awaiter(this, void 0, void 0, function* () {
                return step.beginDialog(dialog.id, step.options);
            }),
            (step) => __awaiter(this, void 0, void 0, function* () {
                let bot = yield this.spawn(step.context,ds);//let bot = yield this.spawn(step.context);
