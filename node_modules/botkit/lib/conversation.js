                                ...
                                {// start $$$ and $$$$ case 
                                    if (condition.pattern.charAt(3) != '$') {// regex: '$$$location&gigi||luigi  patt='gigi||luigi'   ioafild='location'
                                        ...
                                        if (ioa > 0) {
                                            ...
                                            if(ioa>7&&ioafild.substring(0,5)=='vars.'){// $$$vars.....:val , ioafild='vars....' patt='val'
                                            if(state.dir.cond[entity]&&state.dir.cond[entity].term){// the var to take as input , ex ''vars.result' x user text 
                                            let term =looseJsonParse(null, step.values, step,ioafild,null,null);// test with looseJsonParse(null, step.values, 'vars.result',null,null);
                                            if(term)
                                            tomatch=term;
                                            }
                                            ...
                                            console.log(' try matching condition  :  $$$amodel:pattern . amodel: ', ioafild, ' matches item: ', tomatch, '  pattern to test: ', patt);
                                            }
                                        ...
                                        if(!matcher){// special matcher is missing , so  a std matcher regex
                                         ...
                                         ii=stdmres.storeMId// the matching index in ,pArray/pattArray
                                          ...
                                          if(stdmres.notest)notest=true;
                                            } else console.error('probably ERROR  cant run     std Match er    on entity ' ,entity,'  as cant find staff ')
                                    /* moved to stdMatch()
