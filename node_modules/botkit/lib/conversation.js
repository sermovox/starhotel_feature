            ...
            let goonDir = true,// default : do goon(mixedinitiuative) tech
             ...
             if (
                ...
                ////// WARNING   is_prevCollKey = ' ' +  prevName    ?????????????????
                 ...
                 console.log('\n\n ***** PROCESSING previous  ONSTEP ', prevName, '  at current index: ', step.index, '  thread : ', step.thread, ' cmd : ', dc.activeDialog.id, ', om TEXT RESULT : ', step.result, ', answered to a prompt issued in previous step(if -1 it is the step lauching the begindialog/goto thread) index :', msgSent);
                 ...
                 if (msgSent < (step.index - 1))// msgSent is the step to which the user answered ,
                     ...
                     console.log('  NB  GOON detected a previous user answer : so text result , to test previous step conditions at index: ', step.index - 1, ', is the user response to a prompt msg promped at a previous ask index: ', msgSent);
                 else{
                    ...
                    // instead : 
                      console.log('  NB  GOON not detected a previous user answer :');
                 if(step.index>0)console.log('   so test step condition on the user answer (text result) to template of same step . so text result , to test previous step conditions at index: ', step.index - 1, ', is the user response to the prompt msg promped at index: ', msgSent);//, '==', step.index - 1);
                    else console.log('  in this step 0 answer to previous step template , step ' , msgSent ,' will be test on step 1');
                ...
                if(!step.result)msgSent=-1000;
                 console.log('\n\n ***** PROCESSING previous  ONSTEP ', prevName, '  at  current index: ', step.index, '  thread : ', step.thread, ' cmd : ', dc.activeDialog.id, ', on OBJ RESULT , so result is not a user answer to current or previous step template but  coming from GOTOTHREAD/REPLY(result=null)/REPLACEDIALOG ( also returning from child)) is available. ',
                 '\n infact msgSent=',msgSent,'  <= -777 . nb -777 is a OBJ , -1000 is null (a replay request )');
                 if (is_prevCollKey) console.log('   On Previous step  ', is_prevCollKey, ' registered onchange will be fired on result ');
                 if (is_prevColl_Opt) console.log('   and registered  conditions  that can work on a not text result ($$$ and $$$$) ($$ are processed but stdmatcher works only on text result), will be checked on obj result ');
             }
        // a good point to stop watching 
           if(lv>2)console.log('\n   - vars is , ',step.values);
            ...
            if (step.result && previous && previous.collect) {// TESTING CONDITION  IF not null RESULT , step >0, ASK COLLECTING  !
                ...
                // logging 
                function ttest(tomatch,test){// tomatch MUST be text only !!!!!!!!!!!!11  , so test must be a regex string !!
                    ...
                    // experimental : only in case $xmodel:..§.....
                    ...
                    if(test.charAt(0)=='$'){
                        ...
                        let term =looseJsonParse(null, step.values, step,avar,null,null);// test with looseJsonParse(null, step.values, 'vars.result',null,null);
                        let rett;
                        if(term)rett= ['got',term];
                        //}
                        return rett;
                        }              
                    }
                    ...
                    */
                        ...
                        if(condition.type.substring(0,5) === 'macro') {
                            ...
                            if(!(keyDir.cond[entity])&&condition.macro){// add this directive only if is void or add staff also  was not void ??
                                ...
                                if(condition.macro){
                                    ...
                                    if(condition.macro.indexOf('&&')>0){condition.macro=looseJsonParse(condition.macro,Object.assign({},step.values));// try to calc vars between && && in macro json test :
                                }
                    ...
                    for (p = 0; p < paths.length; p++) {// start CONDITION SCAN
                        ...
                        if (condition.type === 'regex'||condition.type.substring(0,5) === 'macro') {// not string pattern (REGEX_MACRO)
                            console.log('condition matching debug : condition is regex or macro type, pattern:  ', condition.pattern
                            ,'\n vars is : ',step.values
                            );
...
if(condition.type.substring(0,5) === 'macro') {// macro only 
                       ...
                       if(!mgot){//hyt
                            ...
                            if (condition.pattern.substring(0, 2) == '$$' || condition.pattern.substring(0, 2) == '$%') {// $xy cases : todo add a $% that means try match , register , but no stop current condition testing, so no route
                                    ...
                                    if (itr > 0) {// start case model match with a $X(Z) case : start $  then % or$ then optional % or § :
                                        ...
                                        if (condition.pattern.charAt(itr + 1) == ':') {// $%%mod_Serv:: , second ':'  means look in model definition. see excel .....
                                            ...
                                            */
                                            ...
                                            if(!des.match)// force null the entity match if is so in the desiredE
        ...
        // ******* model matching section ends 
                        ...
                        itemSep='&';
                            ...
                            for (ii = 0; ii < itr1.length; ii++) {  // gh for all item in model (pArray keys (itemname) array   or pattArray (array of itemname-pattern)  ) test it 
                                ...
                                if(pArray&&pArray[itemNam].index&&pArray.index>=0){// pArray.index contain the index in rows of the item 
...
// hotel customization 
   ...
   console.log('\n Step ' , step.index ,prevName,' NOW Handling CURRENT  STEP of ', thread.length,' steps on thread ',step.thread);
            ...
            if (step.index < thread.length) {
               ...
               // console.log('convo onstep NOW Handle CURRENT STEP: ' , step.index ,' of ', thread.length,'steps on thread ',step.thread);
                if(step.index>0)console.log('  in this step >0    after tested  condition on previous step ',step.index-1,' with a user answere :',step.result,' to msg  of index ',step.msgSent,' ( if smaller (',recLTurn,') is a older msg then the tested condition !) ');
...
*/
            ...
            if(!step.msgSent >-2)console.error(' RREEDD  WARNING : RELAXED step.msgSent=',step.msgSent,' >-2 condition to work with child obj return matching continue routing.result is :  -777 > obj, -1000 > null  '); 

                console.log(' > Ending current Step at index ' + step.index ,' , after tested previous ask ',prevName,'  with GOON directive:\n NEXT  condition will initially be match with a RESULT (if text is the user answer) not related to this step ( answer to templateprompt  but to a previous prompt',
                    '\n so in next step current RESULT (user answer to a template or OBJ injected by child/resume related to a index < ',step.index,' ) will be used to match the condition on ask in step ',step.index,' ;  THATS NOT user answer to its prompting but current RESULT  that is produced/injected/coming from index ',step.msgSent,' <  ',step.index);
                      //console.log('  because we do not retrying because of A ,\n A means :we dont match the condition on a step of index ',step.index-1 , ' on  msg of index ',step.msgSent,' that was a msg of a step before \n');
                       ...
                       step.state.goon=step.msgSent;  // okok:
                        ...
                        let result;
                        if (step.index >0&&step.result && typeof step.result !== 'string' && state.userTurn)result=state.userTurn;else result=step.result;// 012021 : restore past text result in case of child return in meddle of a thread  
            ...
            }   else console.log(' > Ending current Step Handling , now PROMPTing msg of index  ',step.index,' past user answer allowed on index 0 askname: ',curkey,', goonDir2=',goonDir2,'\n');
