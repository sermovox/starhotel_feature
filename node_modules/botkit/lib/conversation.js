...
class BotkitConversation extends botbuilder_dialogs_1.Dialog {
     ...
     */
    constructor(dialogId, controller,ds_) {
        ...
        if(ds_)this.ds=ds_;else this.ds=controller.dialogSet;
        ...
        this.ds.add(new botbuilder_dialogs_1.TextPrompt(this._prompt));//this._controller.dialogSet.add(new botbuilder_dialogs_1.TextPrompt(this._prompt));
        return this;
    }
    /**
...
//          see fwbase.initCmd.controller.plugins.cms.before()
    ...
    resumeDialog(dc, reason, result) {// rersult is the child values/vars
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            console.log(' resumeDialog called state is ',state,' probably returning from a child :',result&&typeof (result) === 'string');// father matches can contains child properties ???? check it with a breakpoint
         ...
         return __awaiter(this, void 0, void 0, function* () {
          ...
          //  return __awaiter(this, void 0, void 0,pippo);pippo= async function* () {
           ...
           // **************   TODO :                >>>> PERFORMANCE ENHANCE substitute     is_prevCollKey=previous.collect.key; and in log : ',prevName,'
            ...
            if(previous&&previous.collect&&previous.collect.key){is_prevCollKey=previous.collect.key;prevName=' '+previous.collect.key;
            ...
            let msgSent=-777,// an integer , the step to which the user answered (useranswer=state.userTurn), usually set as state.goon. -777 means no  text answer is  available from previous steps
            state=step.state,
            ...
            */
            ...
            console.log('\n\n ***** ONSTEP STARTING  at  index: ',step.index,'  thread : ',step.thread,' cmd : ',dc.activeDialog.id,', text result: ',step.result,', answered to a prompt issued in previous step(if -1 the step lauching the begindialog/goto thread) index :',msgSent);
               if(is_prevCollKey)console.log('   >> Matching  Previous Ask  ',is_prevCollKey,
                // prevName,
               '  firing OnChange '); 
               if(is_prevColl_Opt)console.log('   And checking conditions'); 
               if(msgSent<step.index-1)// msgSent say :
               ...
               else console.log('  NB  GOON not detected, test step condition on the user answer (text result) to template of same step . so text result , to test previous step conditions at index: ',step.index-1, ', is the user response to the prompt msg promped at index: ',msgSent,'==', step.index-1); 
           

            }
            else {// result not string 
                console.log('\n\n ***** PROCESSING ONSTEP ',prevName,'  at  index: ',step.index,'  thread : ',step.thread,' cmd : ',dc.activeDialog.id,' , no user answer to current or previous step template ( coming from GOTOTHREAD/REPLY/REPLACEDIALOG ( returning from child)) is available. infact msgSent is -777 : ',msgSent == -777);
           ...
           '\n   - vars is , ',step.values
            ...
            if (step.result && previous && previous.collect) {// step >0 !
                if (previous.collect.key) {
                    ...
                    */
                    ...
                    if(previous.channelData){
                        ...
                        for (var key in previous.channelData) {// adds custom fields set in previous ask in dir.asks.askkey.cf.custofieldkey=value
                    ...
                    if (step.values[previous.collect.key] && previous.collect.multiple) {
                        ...
                        if(step.result&&typeof step.result !== 'string' &&step.result.matches&&step.result.askmatches){
                            ...
                            if(step.result.matches.return){step.values.matches[previous.collect.key]=step.result.matches.return;
                            console.log(' convo.onStep  found a Return from Child , so copied child matches.return model to matches.',previous.collect.key,'that is :\n',step.result.matches.return);}
                            }
                ...
                // logging 
                function ttest(tomatch,test){// tomatch MUST be text only !!!!!!!!!!!!11
                    ...
                    */
                    ...
                    function setAskDir(entity,previous,condition,state){
                        ...
                        */
                        ...
                        if(condition.type.substring(0,5) === 'macro') {
                            ...
                            if(!(keyDir.cond[entity])&&condition.macro){// add this directive only if is void or add staff also  was not void ??
                                keyDir.cond=keyDir.cond||{};
                    ...
                    for (p = 0; p < paths.length; p++) {// start CONDITION SCAN
                        ...
                        let condition = paths[p], 
                            ...
                            if(lv>2)console.log('\n - TRYING CONDITION MATCHING  on thread: ',thread_name,' ,on PREVIOUS step : index ', step.index-1,', ask ',is_prevCollKey,' . matching condition  number ',p,'\n     condition patt: ', condition.pattern,' condition obj : ', condition,' text_tomatch : ',tomatch);
                        let test,
                        notest = false,
                        patt;
...
if(condition.type.substring(0,5) === 'macro') {// macro only 
                       ...
                       if(!mgot){//hyt
                            ...
                            if (condition.pattern.substring(0, 2) == '$$' || condition.pattern.substring(0, 2) == '$%') {// $xy cases : todo add a $% that means try match , register , but no stop current condition testing
                                ...
                                {// start $$$ and $$$$ case 
                                    ...
                                    pattArray, pArray;//  pattArray='item1-pippo!poppo*&item2-parerino&..',pArray={item1:{patt:'pippo!poppo*',(name/value:item1,),,},item2:{patt:'parerino',,},,}
                                    ...
                                    if (itr > 0) {// start case model match with a $X(Z) case : start $  then % or$ then optional % or § :
                                        ...
                                        if (condition.pattern.charAt(itr + 1) == ':') {// $%%mod_Serv:: , second ':'  means look in model definition. see excel .....
                                            ...
                                            console.log('convo.onstep, condition matching debug : $$ case pattern is $$xxx:: model declared on excel ');
                                            ...
                                            */
                                            ...
                                            // SET the name of the generated selecto model 
                                            ...
                                            // its a model generated at run time (desire.intent/param.cursor...)to select an item of a dyn_ask master/ detail query ((askmatches.someask.param.cursor.resModel))  (same name of the ask !!!!!!!!!!)
                                                ...
                                                // des;    //itself : write the selection on the same desired ( matched=best when selected become matched='matched')        
                                                
                                                
                                            // reset the entity just in case alredy matched in past 
                                            if (step.values.matches[entity]) {// attach the sel model to matches
                                            ...
                                            console.log('convo.onstep, condition matching debug : $$ case pattern is $$xxx:> , desire model is (ask)matches.xxx , sel model is matches.ask.param ');
                                           ...
                                           // 
                                            ...
                                            console.log('convo.onstep, condition matching debug : $$ case pattern is model declared inline , matcher detected: ', matcher!=null);
                                    ...
                                    if (!matched) {// start  if didnt match do it  , or retest , or chmatch to do , do it 
                                        ...
                                        if(!matcher){// special matcher is missing , so  a std matcher regex
                                           ...
                                           let stdmres=stdMatch(tomatch,pArray,pattArray,linematch,step,entity);// returns {matched,path,storeVal,notest,storemat};
                                          ...
                                          matched=stdmres.matched;path=stdmres.path;
                                    ...
                                    let itemSep='&',nameSep='-';
                                        ...
                                        */
                                        ...
                                        if(matcher=='dynMatch')matchTyp=0;
                                        else  if(matcher=='intMatch')matchTyp=1;
                                        else  if(matcher=='dynQuery')matchTyp=2;
                                        ...
                                        if(state.dir){// changes the text to match with service; nb seems too  :  state.dir = session.dir 
                                            if(state.dir.cond[entity]&&state.dir.cond[entity].term){// the var to take as input , ex ''vars.result' x user text 
                                                let term=looseJsonParse(null, step.values, step,state.dir.cond[entity].term,null,null);// test with looseJsonParse(null, step.values, 'vars.result',null,null);
                                                if(term)
                                                tomatch_=term;
                                            }
                                        }    
                                            ...
                                            if(this._vcontroller.fwCb.askS[matcher]){// ASWQ entity matcher interface.  TODO test this way 
                                             ...
                                             matched_=yield this._vcontroller.service[matcher](tomatch_,this.id,previous.collect.key,entity,step,
                                                ...
                                                (val)=>{// callback,  the helper that call service will cal this cb so i can set the results of service procedure ,
                                                    ...
                                                    //  service returns :   [] or arow in the format required by this matcher type ( so intent/entity/query/atomic, see formatx.txt )
                                                    // val={meta,rows};  , meta=?
                                        ...
                                        }else if(matchTyp=1){// matchTyp controll he il matcher e' ok x intent resolver
                                                ...
                                                // like ent :matched_=yield this._vcontroller.fwCb.askS[matcher].call(this._vcontroller.service,tomatch,this.id,previous.collect.key,entity,step,(val)=>{res=val[0];});// a cb to set vars in template js code .general int , returns a match obj , see dynMatch in onchange.js
                                                 ...
                                                 matched_=yield this._vcontroller.service[matcher](tomatch_,this.id,previous.collect.key,entity,step,
                                                    ...
                                                    if(res){ // res.conf>0.5 && res.entities .....
                                                       ...
                                                       if(mm){// take first intents
                                                           res.match=res.vmatch=val.intents[0].name;res.matched='match';//// std matcher indicator x intent/entity
                                        ...
                                        */
                                        ...
                                        if (matched) {
                                            ...
                                            addMatcRes(false, entity, null, ii, linematch, rematch, null, param, null,null,pattArray);//24112020 added def in desire selection with :>
                                                                                                ...
                                                                                                //   if dont match , anyway set the return model to default desire  item, we can see if is really matched for example looking at matched
        ...
        // ******* model matching section ends 
                        ...
                        function stdMatch(tomatch,pArray,pattArray,linematch,step,entity){//  a std matcher regex
                        ...
                        itemSep='&';
                            ...
                            if (pArray)// ( the pattArray format in case $$aaskquery:> )
                             itr1 = Object.getOwnPropertyNames(pArray);
                            else {
                                // TODO TODO todo : do the same on mod_feature branch
                            ...
                            if( itr1.length>0){// itr1= array of itemName , really voice name 

                                let fe, gr=1,// regex group
                                freg,doL=false,fout;// filter out, only the filteroutput,the relevant entity content, will be matched against the model
                                ...
                                if(itr1[0].charAt(0)=='§'){// set item and patt separator in case of pattArray format 
                                     if((fe=itr1[0].indexOf('§',2))>0){
                                       ...
                                       if(fm&&Array.isArray(fm)&&fm.length >= gr && Boolean(fm[gr]))
                                            {doL=true;tomatch=fout=fm[gr];
                                                console.log(' condition try matching : $xmodel:§zR§inlinemodeldeclaration case recognized, inlinemodeldeclaration model will be tested on regex R, group z, match: ',tomatch);}
                                    }}else doL=true;
                            if(doL)
                            for (ii = 0; ii < itr1.length; ii++) {// gh for all item in model test it 
                                // start  condition try matching 
                                ...
                                if (!notest && (rematch = ttest(tomatch, patt))) {// rematch means 'match result'. match if ttest returns a not null rematch. rematch[1]>matches.entity.
                                    ...
                                    if(ii==0&&storemat == 'value'){// if the itemname is 'value' : fills this item with regex group : storeVal = matching group
                                    if (!fout){
                                    ...
                                    else{// fout : new filter and get value impl.
                            ...
                            return {matched,path,storeVal,notest,storemat,storeMId:ii};
...
//////////////////////////////////////////    end put in onchange !!!!!
                    ...
                    // ****************  step.msgSent null ??????????????????????
                    console.log('\n\n  *** FINISHED MATCHING CONDITION ON key: ' , previous.collect.key,' cmd : ',dc.activeDialog.id,' cond number: ',p,'       ****  MATCH = ', path != null,' ( action : ',(path||{action:''}).action,
                    ...
                    if (!path) {// start !PATH , previous  conditions dont matched
                        ...
                        console.log(' > so previous  conditions , index  ',index,' dont matched,   msg was sent before this condition test step msg : ', step.msgSent < step.index - 1);// sent if msgSent=1 index=2, msgSent=1 index=3
...
// hotel customization 
   ...
   console.log('Step ' , step.index ,prevName,' NOW Handling CURRENT  STEP of ', thread.length,' steps on thread ',step.thread);
            ...
            if (step.index < thread.length) {
               ...
               // console.log('convo onstep NOW Handle CURRENT STEP: ' , step.index ,' of ', thread.length,'steps on thread ',step.thread);
                ...
                else console.log('  in this step 0 ',prevName,'     a  user answer : ',step.result,' may have recovered  from a prompt issued at index ',step.msgSent, ' so to a previous prompt : si/no : ',(index <0&&index>-777));
...
*/
                       ...
                       console.log(' > Ending current Step at index ' + step.index ,' >0, after tested ',prevName,'  with GOON directive: so in next step this user answere will be used to match the condition on this step ; without prompting but using  a user answer to a previous prompt , of index <  ',step.index);
                       ...
                       if(step.index >0)console.log('  because we  matched a non def  condition on a step of index ',step.index-1 , ' on  prompt of index ',step.msgSent,' that is a step before this step \n',
     ...
     // return Function('"use strict";return (' + obj + ')')();
        ...
        try{calc=eval( fc);// fc has in scope  vars, and vars.text , return the last expression evaluated ( interessa vedere se e' false o no )
            ...
            if (e instanceof SyntaxError) {
                console.error(' looseJsonParse :  match failed x SYNTAX ERROR: ',e.message);
                calc=null;
            }
        }
        if(calc){
