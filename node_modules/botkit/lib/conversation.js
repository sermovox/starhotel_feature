        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            */
            ...
            if(state.goon&&state.goon<=step.index-1){
                ...
                '\n   NB setgoon is set in TTT when goon with a user answere referring to a older msg index: ',state.goon);
                state.goon=null;// reset
            }
            // else msgSent=state.goon;???????????
            }
            console.log('if result is text (msg step> -777 (goto thread/childreturn-replace...  dialog))  it can be the answer to a msg of a step previous than previous step, msg step is :  ',msgSent);
            ...
            if (step.result && previous && previous.collect) {
                if (previous.collect.key) {
                    ...
                    yield this.runOnChange(previous.collect.key, step.result, dc, step);// can set askmatches[previous.collect.key].param 
                    console.log(' +++++++++++  conversation runned ask ',previous.collect.key,' onchange , now vars is : ',step.values);
                ...
                if (previous.collect.options) {
                   ...
                   // step.values.askmatches[previous.collect.key]=null;// reset all match key fields but not param (set by onchange) at start conditions loop p
                    if(step.values.askmatches[previous.collect.key]&&step.values.askmatches[previous.collect.key].param)
                    step.values.askmatches[previous.collect.key]={param:step.values.askmatches[previous.collect.key].param}
                    else step.values.askmatches[previous.collect.key]=null;
                    for (let p = 0; p < paths.length; p++) {

                        let condition = paths[p];
                        if(lv>2)console.log(' step Previous Mngmnt, thread ',thread_name,' ,step index ', step.index,' in previous mngment try matching condition  number ',p,', condition : ', condition);
                        let test;
                        let notest = false;
                        let patt;
                            ...
                            */
                            ...
                            if (condition.pattern.substring(0, 2) == '$$' || condition.pattern.substring(0, 2) == '$%') {// todo add a $% that means try match , register , but no stop current condition testing
                                ...
                                if (condition.pattern.substring(0, 3) == '$$$')// recover the value  values.matches[ioafild]
                                {
                                    ...
                                    prevMat;
                                    let cursor,param;// a desire ask query result field , in case a selection will be required by some condition :$$desire_ask:; 
                                    ...
                                    if (itr > 0) {// a model match with entity name declared : $XYentityname
                                        ...
                                        if(condition.pattern.charAt(itr+1)==':'){
                                            
                                            
                                            // excel declaration : of entity mod_Serv
                                            ...
                                            entity = condition.pattern.substring(2+sk_, itr);// mod_loc
                                        ...
                                        }else if(condition.pattern.charAt(itr+1)==';'){// selection on a ask whose onchange got a query result cursor : step.values.askmatches[desiredE].cursor
                                            
                                            /* for cursor format see bot.js the ask onchange ,
                                            remember 
                                                - the cursor can be passed to this resolver ask that will find a single match , so in 
                                                        addMatcRes()  will complete the setting of :
                                                    ...
                                                    param.match=blResNam;//=blResItem[1];//  name 
                                            ...
                                            */
                                            ...
                                            //          nb the normal  ask containing this ':;' resolver condition will be preeceeded by a msg 
                                            ...
                                            */
                                            ...
                                            let des=step.values.askmatches[desiredE];
                                            if(des&&des.param.cursor&&des.param.cursor.resModel){
                                                param=des.param;cursor=des.param.cursor;// des.param.cursor={rows:res.rows,resModel:{}}
                                                            /*
                                                                 resModel={val1:{
                                                                                 patt:regexstr,
                                                                                vname:'pippo'
                            ...
                            */
                            ...
                            if (storemat) {// if there is the entity value , so in case of $$ , a model match 
                               ...
                               if(chmatch){// also here do routing to new match also if was the same as before
                                if(linematch)path = condition;// do not route if is $%  0503
                                addMatcRes(true,entity,storemat,ii,linematch,rematch,null,param);// rebuild anyway

                               }
                               else   addMatcRes(true,entity,storemat,ii,linematch,rematch,null,param);// linematch means to route on this condition , if maches
                            ...
                            function addMatcRes(mat,// true : matched
                                        ...
                                        ,routing// routing=linematch is true if not $% case ( not routing case )
...
*/
                                ...
                                step.values.askmatches[previous.collect.key]=step.values.askmatches[previous.collect.key]||{matches:[],// [mod_ent1,mod_ent5,,] also condition with $%
                                ...
                                // if(step.values.askmatches[previous.collect.key][mf])
                                    if(entity)step.values.askmatches[previous.collect.key][mf].push(amatch);
                                    if(routing){ step.values.askmatches[previous.collect.key].match=amatch;
                                                    step.values.askmatches[previous.collect.key].mId=amatchId;}
                                // else step.values.askmatches[previous.collect.key][mf] = {key:entity};// first value
                                //step.values.askmatches[previous.collect.key].match += entity;// += '|'+entity in case of multimatch. register too the step was  matched in favor of entity 
                                ...
                                if(entity){// manage matching in condition with model 
                                    if(mat){
                                    step.values.matches[entity] ={match: storemat};// register under values.matches.entity=itemvalue
                                    step.values.matches[entity].mid=storeMId;
                                    }else step.values.matches[entity] ={match: null};// tested but not matched 
                ...
                */
                                       ...
                                       param.group.sel=param.cursor.rows[storeMId];
                                       ...
                                       step.values.matches[entity].vmatch=param.vmatch ;// pass in param ??

                                        // attacjh param to model ( AND to this normal ask ? )
                                       step.values.askmatches[previous.collect.key].param=param;
                                       step.values.matches[entity].param=param;
...
// hotel customization 
   ...
   //             console.log(', ** this previous step of index 0, recovering last user turn : with isstringresult =',typeof step.result == 'string',' isresultNull: ',step.result ==null);//,' . String: ',!step.result instanceof String);
            ...
            if (step.index < thread.length) {
                ...
                console.log('convo onstep now Handle the current step ' + step.index ,' of ', thread.length,'steps on thread ',step.thread);
                ...
                else console.log('  step 0  recovered a  user answer to a previous step (in another thread) :',step.result,' to a msg  of index <0 : ',step.msgSent);
