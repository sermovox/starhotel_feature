...
// todo  handle a invalid regular expression (UnhandledPromiseRejectionWarning: SyntaxError)  , see https://thecodebarbarian.com/unhandled-promise-rejections-in-node.js.html
                ...
                if (previous.collect.options) {
                   ...
                   // step.values.askmatches[previous.collect.key]=null;// reset all match key fields but not param (set by onchange) at start conditions loop p
                    ...
                    function setAskDir(entity,previous,condition,state){// set directive in state.dir.asks[previous.collect.key] , found as macro field in a ask model condition ($.....) , if a model is many times called , only first occurence will get macro 
                        // return the  matchername  if found as directive in condition.macro.matcher
                        // TODO
                        // check if dir are also set in excel . In this case they are alredy copyed into state.dir ? if they conflict with this  dir in macro .....
                        ...
                        if(condition.type.substring(0,5) === 'macro') {
                            // fills directive field 
                                                       ...
                                                       // session.dir=state.dir={asks:{ 
                                                        ...
                                                        //                      }
                            if(!state.dir)return;
                            let keyDir=state.dir.asks[previous.collect.key];
                            keyDir.cond=keyDir.cond||{};//keyDir=state.dir.asks[previous.collect.key];
                            if((!keyDir.cond)&&condition.macro){// add this directive only if is void or add staff also  was not void ??
                                //var str = '{ "name": "John Doe", "age": 42 }';var obj = JSON.parse(str);
                                keyDir.cond[entity]= JSON.parse(condition.macro);
                                ...
                                if(keyDir.cond[entity].matcher)return keyDir.cond[entity].matcher;// keyDir.cond.entmodel.matcher=state.dir.asks[previous.collect.key].cond.entmodel.matcher;
                        }else{
                            if(keyDir.cond[entity]&&keyDir.cond[entity].matcher)return keyDir.cond[entity].matcher;
                        }
                    }
                                ...
                                if (condition.pattern.substring(0, 3) == '$$$')// recover the value  values.matches[ioafild]
                                {
                                    ...
                                    if (itr > 0) {// and < 20  , a model match with entity name (<entityname>) declared : $X(Z)<entityname>:...  Z can be % or § ,  X can be $ or % 
                                        ...
                                        if (condition.pattern.charAt(itr + 1) == ':') {// &%%mod_Serv:: , second ':'  means look in model definition. see excel .....
                                           ...
                                           // cant be a non regex matcher if inline declaration !
                                           // matcher=
                                    ...
                                    let itemSep='&',nameSep='-';
                                        ...
                                        if(state.dir){// state.dir.asks.....  must have been declared as directive !  , the matcher directive  , usually set in macro condition , must have an entry in 
                                           ...
                                           // async
                                            let matched_=null // a obj , x example :{match,rows:[[vmatch,id,rowspatt,shortD,bldata1,bldata2,,,],,],type='dynEnt/dynCursor,,,,,}


                                            if(this._vcontroller.fwCb.askS[matcher]){
                                            /// GHTR

                                            // set a std default matcher (!= regex )
                                            if(!this._vcontroller.fwCb.askS[matcher])matcher='dynMatch';// the std dyn matcher, can be customized
                                            if(this._vcontroller.fwCb.askS[matcher])// a registered ( in service or in cms) injected directive service
                                            matched_=this._vcontroller.fwCb.askS[matcher](tomatch,entity,step.values.excel,step.values.matches[entity],step.values.matches);// returns a match obj , see dynMatch in onchange.js
                                           ...
                                           // state.dir.asks[previous.collect.key].cond[entity].notMatchPrompt=matched_.nmp;
                                            ...
                                            if(matched_&&matched_.match){ matched = true;
                                                if (linematch) path = condition;// do not route if is $%
                                                if(matched_.vmatch)step.values.__conf +=matched_.vmatch;
                                                storemat=matched_;// 072020 matching in a not standard matcher ( static model enitity described in excel ) is a obj with .match and .type
                                            }
                                            }
