...
class BotkitConversation extends botbuilder_dialogs_1.Dialog {
    ...
    constructor(dialogId, controller) {
        ...
        if(this._vcontroller){this.mustacheF=this._vcontroller.mustacheF;// store singleton for mustache function (  cant be set in status (/vars/values) as it wont store/persist functions )
           ...
           console.error=function(...args){consorig(...args);
            elogs(...args)};
           // winston to use
...
*/
           ...
           // **************   TODO :                >>>> PERFORMANCE ENHANCE substitute     is_prevCollKey=previous.collect.key; and in log : ',prevName,'
            ...
            let goonDir = true,// default : do goon(mixedinitiuative) tech
                ...
                logsout+='\n *****   at current index: '+ step.index+' PROCESSING previous  ONSTEP of name'+ prevName+ '  thread: '+ step.thread+ ' cmd: '+ dc.activeDialog.id+' counter: '+step.values.session.dir.count;
   ...
   console.log('\n Step ' , step.index ,prevName,' NOW Handling CURRENT  STEP of ', thread.length,' steps on thread ',step.thread);
            ...
            if (step.index < thread.length) {
               ...
               // console.log('convo onstep NOW Handle CURRENT STEP: ' , step.index ,' of ', thread.length,'steps on thread ',step.thread);
                if(step.index>0)console.log(' NOW Handle CURRENT STEP: in this step >0    after tested  condition on previous step ',step.index-1,' with a user answere :',step.result,' to msg  of index ',step.msgSent,' ( if smaller (',recLTurn,') is a older msg then the tested condition !) ');
                else console.log('  NOW Handle CURRENT STEP: in this step 0 ',prevName,'     a  user answer : ',step.result,' may have recovered  from a prompt issued at index ',step.msgSent, ' so to a previous prompt : si/no : ',(step.msgSent <0&&step.msgSent>-777));
...
*/
            ...
            if(!step.msgSent >-2)console.error(' RREEDD  just WARNING : RELAXED goon because step.msgSent, >-2 meand the previus message comes from the ask that goon to this th or a this th ask : \n step.msgSent=',step.msgSent,' >-2 condition allows to work with child obj return matching continue routing. '); 
                ...
                // returned , exit !!!
                 ...
                 console.log(' > Ending current Step Handling WITHOUT GOON  support on passed answer , now prompt the ask msg:',
     ...
     // return Function('"use strict";return (' + obj + ')')();
        ...
        try{calc=eval( fc);// fc has in scope  vars, and vars.text , return the last expression evaluated ( interessa vedere se e' false o no )
            ...
            if (e instanceof SyntaxError) {
                ...
                calc=null;
            }else console.error(' looseJsonParse :  match failed x ERROR: ',e.message);
        }
        if(calc){
