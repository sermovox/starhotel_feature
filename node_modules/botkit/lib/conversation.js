            ...
            */
            ...
            console.log('\n\n ***** ONSTEP STARTING  at  index: ',step.index,'  thread : ',step.thread,' cmd : ',dc.activeDialog.id,', text result: ',step.result,', answered to a prompt issued in previous step(if -1 the step lauching the begindialog/goto thread) index :',msgSent);
               ...
               console.log('  NB  GOON detected : so text result , to test previous step conditions at index: ',step.index-1, ', is the user response to a prompt msg promped at a previous ask index: ',msgSent ); 
                ...
                console.log('\n\n ***** PROCESSING ONSTEP ',prevName,'  at  index: ',step.index,'  thread : ',step.thread,' cmd : ',dc.activeDialog.id,' , with a not string result coming from GOTOTHREAD/REPLY/REPLACEDIALOG ( returning from child). infact msgSent is -777 : ',msgSent == -777);
                                ...
                                {// start $$$ and $$$$ case 
                                    ...
                                    if (itr > 0) {// start case model match with a $X(Z) case : start $  then % or$ then optional % or § :
                                        ...
                                        if (condition.pattern.charAt(itr + 1) == ':') {// $%%mod_Serv:: , second ':'  means look in model definition. see excel .....
                                            ...
                                            */
                                            ...
                                            entity = previous.collect.key;// the generated model name that will do the desire rows selection (same name of the ask !!!!!!!!!!), ;
                                            ...
                                            des = askmatches[desiredE]||step.values.matches[desiredE];// search x runtime model
                                    ...
                                    let itemSep='&',nameSep='-';
                                        ...
                                        */
                                        ...
                                        if(state.dir){// chages the text to match , seems too  :  state.dir = session.dir 
                                        ...
                                        if(matchTyp==0){// > ENTITY MATCH MATCHER case (matchTyp=0): service[matcher](=dynMatch()) must meet ASWQ entity matcher interface : see dynMatch()
                                           ...
                                           // state.dir.asks[previous.collect.key].cond[entity].notMatchPrompt=matched_.nmp;
                                            ...
                                            // if( get query master cursor .....)
                                            // usually the query result , cursor, will fill a askmatches.param  in def format
                                            // so if the query result is in std apy , result={reason,rows} we must map in askmatches format 
                                            // or if we want to resolve the query master list with a child with its params (vars used in child like template context and flags used in conditions ,,, )
                                            // the askmatches.child.param must be completed with askmatches.child.params (usually got from excel)  by the condition that fires the child 
                                            mT='Query';
                                            // will call service dynQuery() returning askmatches
                        ...
                        itemSep='&';
                            ...
                            for (ii = 0; ii < itr1.length; ii++) {// gh for all item in model test it 
                                ...
                                if (!notest && (rematch = ttest(tomatch, patt))) {// rematch means 'match result'. match if ttest returns a not null rematch. rematch[1]>matches.entity.
                                    ...
                                    if(ii==0&&storemat == 'value'){// fills this item with regex group 
                                    if (!fout){
