    ...
    onStep(dc, step) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            /* cases on step.result :
                ...
                - step.values               gotothread will use goon to recover userturn , return from child
            ...
            */
            ...
            console.log('\n onStepstarting , **  index: ',step.index,' setting last user turn thread : ',step.thread,' step.status.goon : ',step.state.goon, ' text/result : ',step.result ,' conditions ',typeof step.result === 'string',' or ',step.result instanceof String);

            let msgSent,state=step.state;// 
            ...
            if(state.goon&&state.goon<=step.index-1){msgSent=state.goon;// usually step run with a user answare to msg of step with index-1 
                console.log('       , state.goon found, thread: ',step.thread,' index is ' + step.index ,' setgoon is set in TT when goon with no step msg : ',state.goon,' text is ',step.result);
            ...
            if (step.result && previous && previous.collect) {
                ...
                if (previous.collect.options) {
                    ...
                    console.log('\n previous finished condition matching, at index: ',step.index,' , msgsent: ',step.msgSent,' ,condition matched : ',path!=null,' ,default_path=   ',default_path);

                    if (!path) {

                        console.log(' previous  condition dont matched,   msg was sent in past  ',step.msgSent>=step.index-1);// sent if msgSent=1 index=2, msgSent=1 index=3
                        ...
                        if(default_path.action=='next'&&step.msgSent&&step.msgSent<step.index-1){// in case of continue we garantee 1 msg prompt to retest the condition
                            console.log('   previous  condition at index',step.index-1, ' dont matched and as msg was of a step before ( index ',step.msgSent,' ) we retry the step at index ',step.index-1
                            ...
                            path=Object.assign({}, dc.activeDialog.state.values);path.action='repeat';
...
// hotel customization 
            ...
            */
                let recLTurn=false;
            if(step.index==0){// in starting a new thread (after a goto) try to recover last user turn answere in the current cmd
                ...
                // at index 0 we can have null if retrying or a beging dialog  (must not recover) or object  if coming from a goto thread (should try to recover)
                    console.log(', ** previous step 0, recovering last user turn :',typeof step.result != 'string',' and ',step.result !=null,' . String: ',!step.result instanceof String);

                if(step.result !=null&&typeof step.result != 'string' ){
                    // so is obj ( coming from goto thread), 
                    ...
                    console.log('convo onstep Handle the current index 0 step ,recovering last user turn  ' + step.index ,' processing previous condition at index ', step.index-1,' on answere to msg  of index ',step.msgSent,' recovering text is ',step.state.userTurn);
 
                    // now we can change the result : 
            ...
            if (step.index < thread.length) {
                ...
                console.log('convo onstep Handle the current step ,step.index is ' + step.index ,' processing previous condition at index ', step.index-1,' on answere to msg  of index ',step.msgSent,' text is ',step.result);
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            switch (path.action) {
                ...
                case 'execute_script':
                    ...
                    console.log(' execute script was received ',path);
