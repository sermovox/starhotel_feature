    ...
    onStep(dc, step) {
         return __awaiter(this, void 0, void 0, function* () {
          ...
          //  return __awaiter(this, void 0, void 0,pippo);pippo= async function* () {
           ...
           if(is_prevCollKey)console.log('   and as previous has a key ',is_prevCollKey,' registered onchange will be fired on  obj result'); 
           if(is_prevColl_Opt)console.log('   and registered  conditions  will be checked on obj result (??) ');       
           }
                ...
                if (previous.collect.options) {
                   ...
                   // step.values.askmatches[previous.collect.key]=null;// reset all match key fields but not param (set by onchange) at start conditions loop p
                        ...
                        */
                        ...
                        if(condition.type.substring(0,5) === 'macro') {
                            ...
                            if((!keyDir.cond)&&condition.macro){// add this directive only if is void or add staff also  was not void ??
                                ...
                                keyDir.cond[entity]= JSON.parse(condition.macro);// cmsmacrojsonstring={ "matcher": "dynMatch" }'
                                ...
                                if(keyDir.cond[entity].matcher)return keyDir.cond[entity].matcher;// state.dir.asks[previous.collect.key].cond[entity].matcher="dynMatch"
                            }else if(step.values.excel[entity]&&step.values.excel[entity].matcher) return step.values.excel[entity].matcher;// will be called on MFM
                        }
                    }
                    ...
                    for (let p = 0; p < paths.length; p++) {// start CONDITION SCAN
                        ...
                        if(lv>2)console.log('\n - Step Previous Mngmnt CONDITION MATCHING , thread ',thread_name,' ,step index ', step.index,', in previous ask ',is_prevCollKey,'  try matching condition  number ',p,', condition patt: ', condition.pattern,'\n condition obj : ', condition);
                        let test;
                        let notest = false;
                        let patt;
...
if(condition.type.substring(0,5) === 'macro') {// macro only 
                        ...
                        if(!mgot){//  hyt
                            ...
                            if (condition.pattern.substring(0, 2) == '$$' || condition.pattern.substring(0, 2) == '$%') {// $xy cases : todo add a $% that means try match , register , but no stop current condition testing
                                ...
                                {// start $$$ case 
                                ...
                                else {// $$ case (test and condition route ) or $% (test and not condition route ) or $$% $%% (% in 3rd position means to test anyway )
                                    ...
                                    if (itr > 0) {// start case model match
                                    ...
                                    let itemSep='&',nameSep='-';
                                        ...
                                        if(matchTyp==0){
                                           ...
                                           // async
                                            ...
                                            if(this._vcontroller.fwCb.askS[matcher])matchIn=this._vcontroller.fwCb.askS[matcher];
                                            else if(this._vcontroller.service[matcher])matchIn=this._vcontroller.service[matcher];
                                            if(matchIn){// see comment in 
                                                // start GHTR  

                                            // >>>>>>    MUST BE SAME INTERFACE AS : _vcontroller.service.dynMatch
                                             matched_=await matchIn(tomatch,this.id,entity,step,(val)=>{res=val[0];});// a cb to set vars in template js code .general int , returns a match obj , see dynMatch in onchange.js
                                           ...
                                           // state.dir.asks[previous.collect.key].cond[entity].notMatchPrompt=matched_.nmp;
                                            ...
                                            if(matched_&&res){ matched = true;
                                                ...
                                                // match can be the sme as vmatch
                                                storematch=rows[0];
                                                // add fields so that addMatcRes() knows the type of result to insert 
                                                storematch.type=mT;// storemat.value must exists !!!!
...
*/
                                ...
                                if(entity){// in this condition we  manage matching (vars.matches/askmatches) on model and value entity/value ($$,,)
                                    ...
                                    let isVal=storeVal&&rT==0&&storemat=='value',// a model whose values are the regex group match , not the item name declared in excel
                ...
                */
                    ...
                    console.log('  at index ', step.index,' the previous step ask ',previous.collect.key,'found MATCH = ', path != null,', on user answere :',step.result,' to msg at index :', step.msgSent, ' \ncondition matched :  ' , path, ' , default_path=   ', default_path);
