"use strict";
const AhoCorasick = require('ahocorasick');// npm install ahocorasick
...
// todo  handle a invalid regular expression (UnhandledPromiseRejectionWarning: SyntaxError)  , see https://thecodebarbarian.com/unhandled-promise-rejections-in-node.js.html
                ...
                function ahocorA(tomatch, patt){// https://github.com/BrunoRB/ahocorasick
                    ...
                    let keys;
                    if(patt.indexOf('|')>0)keys='|';else keys=' ';
                    let skeys=patt.split(keys);
                    if(skeys.length>0){
                    let matches=new AhoCorasick(skeys)//['keyword1', 'keyword2', 'etc']); or ' ' if dont find any '|'
                    .search(tomatch.toLowerCase());
        ...
        // ******* model matching section ends 
                        ...
                        itemSep='&';
                            ...
                            for (ii = 0; ii < itr1.length; ii++) {  // gh for all item in model (pArray keys (itemname) array   or pattArray (array of itemname-pattern)  ) test it 
                                ...
                                if(ii==0&&patt.charAt(0)=='@'){// case : ahocorasick , see https://github.com/BrunoRB/ahocorasick
                                ...
                                if (!notest )
                                    if(ahocorasick){
                                        ...
                                        storematV.push(storemat);
                                        score.push(ahocorA(tomatch, patt));// [3,0.75] so 3 kewword match , score is 0.75 , so total kyword to matc was 3/3*4=4
                            ...
                            if(ahocorasick){// choose the max keyword not duplicated found 
                                ...
                                score.forEach((el,ind) => {//score was : [ [ [ 19, [ 'keyword1' ] ], [ 47, [ 'keyword2' ] ] ],,,,,]
                                    ...
                                    if(ms<el[0]){//keywords matches 
                                        ms=el[0];mscor=[];mind=[];mscor.push(el);mind.push(ind);
                                    }else if(ms>0&&ms==el[0]){
                                        mscor.push(el);mind.push(ind);
                                    }
                                });
                                let iy=-1;
                                if(mscor){
                                if(mscor.length>1){// leave common ,undiscriminant?
                                   if(mscor[0][1]<mscor[1][1]){iy=mind[1];
                                    ...
                                    iy=mind[0]; scored=mscor[0][1];
                                }
                                }

                                if(iy>=0){
                                storemat=storematV[iy];
                                  // ok : 
   ...
   console.log('\n Step ' , step.index ,prevName,' NOW Handling CURRENT  STEP of ', thread.length,' steps on thread ',step.thread);
            ...
            if (step.index < thread.length) {
               ...
               // console.log('convo onstep NOW Handle CURRENT STEP: ' , step.index ,' of ', thread.length,'steps on thread ',step.thread);
                ...
                else console.log('  in this step 0 ',prevName,'     a  user answer : ',step.result,' may have recovered  from a prompt issued at index ',step.msgSent, ' so to a previous prompt : si/no : ',(step.msgSent <0&&step.msgSent>-777));
