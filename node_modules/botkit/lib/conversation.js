...
class BotkitConversation extends botbuilder_dialogs_1.Dialog {
    ...
    beginDialog(dc, options) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            let dir={repeat:0,condDefInd:-1,condDefNam:null,maxrep:10};
           ...
           // console.log('\n\n ********************************** ONSTEP STARTING  at  index: ',step.index,'  thread : ',step.thread),' appwrapper : ',step.values.app;
            ...
            if (step.result && previous && previous.collect) {
                if (previous.collect.key) {
                    ...
                    // state.dir={repeat:0,condDefInd:-1,condDefNam:null,maxrep:10};
                    ...
                    if(previous.channelData){
                        ...
                        // copy all the values in channelData fields starting with 'dir-' as dirrective in a array
                    ...
                    */
                    ...
                    if(previous.channelData['maxrep'])if(state.repeat>=previous.channelData['maxrep'])setdef= condDefInd ;
...
// todo  handle a invalid regular expression (UnhandledPromiseRejectionWarning: SyntaxError)  , see https://thecodebarbarian.com/unhandled-promise-rejections-in-node.js.html
                ...
                if (previous.collect.options) {
                   ...
                   // step.values.askmatches[previous.collect.key]=null;// reset all match key fields but not param (set by onchange) at start conditions loop p
                    ...
                    // 092020
                    askmatches=askmatches||{};// to avoid error in case there is not fw support on this cmd
                            ...
                            if (condition.pattern.substring(0, 2) == '$$' || condition.pattern.substring(0, 2) == '$%') {// todo add a $% that means try match , register , but no stop current condition testing
                                ...
                                if (condition.pattern.substring(0, 3) == '$$$')// recover the value  values.matches[ioafild]
                                {
                                    ...
                                    if (ioa > 0) {
                                        ...
                                        tomatch=jVar(ioafild,step.values);//ioafild is the model name , to match  is the model item matched value , not the voice name 
 
                                        console.log(' try matching condition  :  $$$amodel:pattern . the model: ', ioafild, ', matches the itemvalue: ', tomatch, ' , pattern: ', patt);
                                    ...
                                    if(looseJsonParse(null,step.values,step, condition.pattern.substring(4))){ // a function as tostring
                                    ...
                                    // $$dyn_medicine:itemnam1-pattern1&itemname2-pattern2......  
                                    // OR get model  $$dyn_medicine:: 
                                    // OR (pArray) select adynquery   $$dyn_medicine:>
                                    ...
                                    if (itr > 0) {// and < 20  , a model match with entity name declared : $XYentityname
                                        ...
                                        if(condition.pattern.charAt(itr+1)==':'){// &%%mod_Serv:: , second ':'  means look in model definition. see excel .....
                                            ...
                                            */
                                            ...
                                            entity = previous.collect.key;// its a model generated at run time to select an item of a dyn_ask master/ detail query ((askmatches.someask.param.cursor....))  (same name of the ask !!!!!!!!!!)
                                            // reset the entity just in case alredy matched in past 
                                            if(step.values.matches[entity]){step.values.matches[entity].match=null;
                                    ...
                                    else{ // std match against condition pattern, not a model
                                    ...
                                    let itemSep='&',nameSep='-';// default , choose better choices because of typic regex definition contains '-'
                                    if(pArray){// can be also {}
                                        //if(pArray)
                                        itr1=Object.getOwnPropertyNames(pArray);//can be  itr1=[];
                                    }else {
                                    ...
                                    let sar;//=itr1[ii].split(nameSep);//better as the regex can contains '-' , so itr1[ii].indexOf('-');
...
function looseJsonParse(templ,vars,step,jsfunc){// 

     // if there is a jsfunc (or template is void) :
     //         calc jsfunc on current scope (step_vars as param var is in scope !)  and return true if eval returns not false 
     // if jsfunc is void :
     //         substitute in template first or second  ...&&jsfunctionText&&...   with  eval(jsfunctionText) on context=this=vars 

     // nb eval() restituisce il valore dell'ultima espressione valutata !

     //     nb to evaluate eval in different scope ( OR CONTEXT ??????) you can use :
     //                 eval.call(null, 'x'); // x is global , so windows.x
     //                 see : https://2ality.com/2014/01/eval.html
     /*
        so if you want to assign a context use Function and you will mantain the scope !
            var y='myy';
            function myf(){return this}
            var st = new Function('"use strict"; return this.x+y');
            var st1 = new Function('return this.x+y;');
            var txt = "ABCDEFGHIJKLMNOPQRSTUVWXYZ";
            var sln = txt.length;
            let x=5;
            document.getElementById("demo").innerHTML = eval.call(Object.assign({},{x:6}),'"use strict";this.x')+'\n'+ eval('this.x')+'\n'+myf.call(Object.assign({},{x:6}))+'\n'+ eval('x')+'\n'+
            eval.call(null,'x')+'\n'+st1()+'\n'+st1.call(Object.assign({},{x:6}));
            // '\n'+st.call(Object.assign({},{x:6}));
     */
    
    
    // usually extract in cond string a vars . format  xxxx&&vars.excel.....&&yyyy
    // see evalmozilla.js, see https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/eval
    // call  ftempl=looseJsonParse(template,Object.assign({},context.vars,{aa}));
    // template= "some text{{&&let kk="5";vars["colore"];&&}}"// last js expression in &&....&& is calculated
     ...
     let ioa,templP,fc,calc;
      if(!jsfunc&&templ){// if jsfunc is void substitute in template firstor second  ...&&jsfunctionText&&...   with the calc value  jsfunctionText on context=this=vars 
        ioa = templ.indexOf('&&');
        templP=templ.split('&&');
     ...
     // return Function('"use strict";return (' + obj + ')')();
      }else{// // if there is a jsfunc (or template is void) : calc jsfunction on current this extended with vars 
        fc=jsfunc;
