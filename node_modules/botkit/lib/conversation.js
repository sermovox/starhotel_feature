...
class BotkitConversation extends botbuilder_dialogs_1.Dialog {
    ...
    beginDialog(dc, options) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            // Run the first step
            console.log('\n ** beginDialog start runStep , if  default thread assign user text in results, user text:',dc.context.activity.text);
            ...
            return yield this.runStep(dc, 0,th, botbuilder_dialogs_1.DialogReason.beginCalled,result);
        });
    }
    /**
    ...
    onStep(dc, step) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            */
            ...
            '\n   - if result is string , we test previous step condition , so at index ',step.index-1,' ,using  result/answere  that if state.goon ( = ',step.state.goon,' ) < previous index ( = ',step.index-1,' ) can  comes from a step before previous step ',
            '\n   - vars is , ',step.values
            );
            ...
            console.log('if result is text (msgSent=',msgSent,'> -777 (goto thread/childreturn-replace...  dialog))  it can be the answer to a msg of a step previous than previous step !');
                ...
                if (previous.collect.options) {
                   ...
                   // step.values.askmatches[previous.collect.key]=null;// reset all match key fields but not param (set by onchange) at start conditions loop p
                    ...
                    for (let p = 0; p < paths.length; p++) {
                        ...
                        let notest = false;
                        let patt;
                        let tomatch = step.result, storemat = null;// storemat is the item name (or model value)
...
*/
                                ...
                                if(entity){// in this condition we  manage matching using model named entity
                                    if(mat){
                                    ...
                                    if(param&&param.group) {// this condition is selecting a cursor param set by desiredE : param=step.values.askmatches[desiredE].param
                                                            // the model that matches the cursor has the same  name as this ask :
                                                            // entity =previous.collect.key , the name of this ask in testing
...
*/
            ...
            if (index === 0) {
                ...
                console.log('\n ** start runStep , ',dc.activeDialog.id,' , thread ',thread_name,', step ',index);
    ...
    makeOutgoing(line, vars,step) {// vars=step.values
        ...
        if(vars.matches){
            ...
            if(vars.matches[key]&&vars.matches[key].match)matches.push(key);
            }
