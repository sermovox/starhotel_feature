         ...
         return __awaiter(this, void 0, void 0, function* () {
          ...
          //  return __awaiter(this, void 0, void 0,pippo);pippo= async function* () {
            ...
            */
            ...
            console.log('\n\n ***** ONSTEP STARTING  at  index: ',step.index,'  thread : ',step.thread,' cmd : ',dc.activeDialog.id,', text result: ',step.result,', answered to a prompt issued in previous step(-1=begibDialog) index :',msgSent);
               ...
               console.log('  NB  GOON detected : so text result , to test previous step conditions at index: ',step.index-1, ', is the user response to a prompt msg promped at a previous ask index: ',msgSent ); 
                ...
                console.log('\n\n ***** ONSTEP ',prevName,'  STARTING  at  index: ',step.index,'  thread : ',step.thread,' cmd : ',dc.activeDialog.id,' , with a obj result coming from GOTOTHREAD/REPLY/REPLACEDIALOG ( returning from child). infact msgSent is -777 : ',msgSent == -777);
...
// todo  handle a invalid regular expression (UnhandledPromiseRejectionWarning: SyntaxError)  , see https://thecodebarbarian.com/unhandled-promise-rejections-in-node.js.html
                    ...
                    console.log(' convo.ttest() trying matching text ',tomatch,' with regex patt ',test,' got : ', matres);}
                    catch{
                ...
                if (previous.collect.options) {
                    ...
                    */
                    ...
                    for (p = 0; p < paths.length; p++) {// start CONDITION SCAN
                        ...
                        if(lv>2)console.log('\n - TRYING CONDITION MATCHING  on thread: ',thread_name,' ,on PREVIOUS step at index: ', step.index-1,', ask: ',is_prevCollKey,'  try matching condition  number ',p,'\n     condition patt: ', condition.pattern,' condition obj : ', condition);
                        let test;
                        let notest = false;
                        let patt;
                        ...
                        storemat = null,storeVal;// storemat is the item name (if the model is a number or just a regex match is called 'value' ) for menu model(static model + regex match group  in excel) 
                                ...
                                {// start $$$ case 
                                    ...
                                    if (itr > 0) {// start case model match with a $X(Z) case : start $  then % or$ then optional % or § :
                                        ...
                                        if (condition.pattern.charAt(itr + 1) == ':') {// $%%mod_Serv:: , second ':'  means look in model definition. see excel .....
                                            ...
                                            */
                                            ...
                                            console.log('convo.onstep, condition matching debug : $$ case pattern is $$xxx:> model is askmatches.ask.param ');
                                    ...
                                    if (!matched) {// start  if didnt match do it  , or retest , or chmatch to do , do it 
                                        ...
                                        if(!matcher){// special matcher is missing , so  a std matcher regex
                                          ...
                                          matched=stdmres.matched;path=stdmres.path;storemat=stdmres.storemat;storeVal=stdmres.storeVal;if(stdmres.notest)notest=true;
                                        ...
                                        */
                                        ...
                                        if(matchTyp==0){// > ENTITY MATCH MATCHER case (matchTyp=0): service[matcher](=dynMatch()) must meet ASWQ entity matcher interface : see dynMatch()
                                           ...
                                           // async
                                            ...
                                            // >>>>>>    MUST MEET  ASWQ entity matcher interface :  _vcontroller.service.dynMatch
                                            ...
                                            if(this._vcontroller.fwCb.askS[matcher]){// ASWQ entity matcher interface.  TODO test this way 
                                            ...
                                            else if(this._vcontroller.service[matcher]){// ASWQ entity matcher interface. 
                                             ...
                                             matched_=yield this._vcontroller.service[matcher](tomatch,this.id,previous.collect.key,entity,step,
                                                (val)=>{// callback,  the helper that call service will cal this cb so i can set the results of service procedure ,
                                                    ...
                                                    //  service returns :   [] or string in the format required by this matcher type ( so intent/entity/query/atomic, see formatx.txt )
                                                ...
                                                // like ent :matched_=yield this._vcontroller.fwCb.askS[matcher].call(this._vcontroller.service,tomatch,this.id,previous.collect.key,entity,step,(val)=>{res=val[0];});// a cb to set vars in template js code .general int , returns a match obj , see dynMatch in onchange.js
                                                 ...
                                                 matched_=yield this._vcontroller.service[matcher](tomatch,this.id,previous.collect.key,entity,step,
                                                    ...
                                                    if(res){ // res.conf>0.5 && res.entities .....
                                                        ...
                                                        let mm= val.intents[0]&&    val.intents[0];                                  // assume format in formats_x.txt
        ...
        // ******* model matching section ends 
                        ...
                        itemSep='&';
                            ...
                            if( itr1.length>0){
                                ...
                                freg,doL=false,fout;// filter out, the relevant entity content
                            ...
                            for (let ii = 0; ii < itr1.length; ii++) {// gh for all item in model test it 
                                ...
                                if (!notest && (rematch = ttest(tomatch, patt))) {// rematch means 'match result'. match if ttest returns a not null rematch. rematch[1]>matches.entity.
                                    // ok : 
                                    console.log(' \n * MATCH Detected on MODEL ', entity, ' with $$ or $% condition, at thread ', thread_name, ',step ', step.index, ' ,previous ask key ', previous.collect.key, ' ,ask condition index ', p, '\n  matched item : ', storemat, ', index : ', ii, ', regex group extracts: ', rematch);
                                    step.values.__conf = step.values.__conf || '';// confirm  the user matched something 
                                    //if(data !== null && data !== ''  && data!==undefined) {}  You can use below simple code
                                    // if(Boolean(value)){ 
                            ...
                            return {matched,path,storeVal,notest,storemat};
