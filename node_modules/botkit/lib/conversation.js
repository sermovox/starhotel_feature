          ...
          //  return __awaiter(this, void 0, void 0,pippo);pippo= async function* () {
            ...
            */
            ...
            console.log('\n\n ***** ONSTEP STARTING  at  index: ',step.index,'  thread : ',step.thread,' cmd : ',dc.activeDialog.id,', text result: ',step.result,', answered to a prompt issued in previous step(-1=begibDialog) index :',msgSent);
               if(is_prevCollKey)console.log('   On Previous step  ',is_prevCollKey,prevName,' registered onchange will be fired on text result '); 
                                ...
                                {// start $$$ case 
                                    ...
                                    let itemSep='&',nameSep='-';
                                        ...
                                        - convo std/notstd matcher usually is , a service (a match helper like intMatch,dynQuery or dynMatch) that convert the format of a external run rest service (run_jrest())
                                            ...
                                            , so matcher according its type ( query,entity, intent) will add/ update some fields and pass matched,storemat/storeVal to addMatcRes(matched,storemat/storeVal ) that according to type will:
                                           ...
                                           // async
                                            ...
                                            if(this._vcontroller.fwCb.askS[matcher]){// TODO test this way 
                                              ...
                                              matched_=yield this._vcontroller.fwCb.askS[matcher].call(this._vcontroller.service,tomatch,this.id,previous.collect.key,entity,step,(val)=>{res=val[0];});// a cb to set vars in template js code .general int , returns a match obj , see dynMatch in onchange.js
                                                ...
                                                console.log('convo.onstep, condition matching debug : matcher got in session.dir, type Ent, so try matching entity ',entity,' with   service   ',matcher,'\n with Ent API: (text,id,key,entity,cb([{_doc:result}||result,,]||{_doc:result}||result)), result should be cursor in std format, returns :true/false ');
...
//////////////////////////////////////////    end put in onchange !!!!!
                    ...
                    // ****************  step.msgSent null ??????????????????????
                    console.log('\n\n  *** FINISHED MATCHING CONDITION ON key: ' , previous.collect.key,' cmd : ',dc.activeDialog.id,' cond number: ',p,'  with a Condition MATCH = ', path != null,' ( action : ',(path||{action:''}).action,
