        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (step.result && previous && previous.collect) {
                ...
                if (previous.collect.options) {
                    ...
                    console.log('\n previous at index: ',step.index,' , msgsent: ',step.msgSent,' ,condition matched : ',path!=null,' ,default_path=   ',default_path);
                    ...
                    if (!path) {
                        ...
                        if(default_path.action=='next'&&step.msgSent<step.index-1){// in case of continue we garantee 1 msg prompt to retest the condition
            ...
            if (step.index < thread.length) {
                ...
                step.state.goon=null;// ever true for now
                // if(step.values.goon&&step.values.goon==true&&step.index>0)
                if(step.index>0&&step.msgSent>=step.index-1)// if index > 0 do not send msg, it will be sent as repeat next step if dont match
                {     console.log('Handle the current step , goon next step ',step.index+1, ' because we do not retrying because of A :we dont match the condition on a step of index ',step.index-1 , ' on  msg of index ',step.msgSent,' that was a msg of a step before ');
                     step.state.goon=step.msgSent;// inform runstep that index-1 msg was not sent 
...
// hotel customization 
            ...
            if(state.goon||state.goon>=index-1){msgSent=state.goon;// usually step run with a user answare to msg of step with index-1 
                console.log('\n runstep thread: ',thread_name,' index is ' + index ,' goon is ',state.goon,' text is ',result);
