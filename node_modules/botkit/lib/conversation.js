...
// pistola 
let logsout;// wrk log var used to build logs strings to log with console,console_ or logs
...
let logs,// the log into file , production logger tracking conversations . in constructor : logs=this._vcontroller.vFw.logs;
console_={log:// console_   in production change all:   console.log( > if(false)console.log(..). then only console_.log can write to console !
///function (frst,...args){
    function (frst,txt){
    ...
    console.log(txt);///   console.log(args);
...
class BotkitConversation extends botbuilder_dialogs_1.Dialog {
    ...
    constructor(dialogId, controller) {
        ...
        if(this._vcontroller){this.mustacheF=this._vcontroller.mustacheF;// store singleton for mustache function (  cant be set in status (/vars/values) as it wont store/persist functions )
            logs=this._vcontroller.vFw.logs;
        }
          ...
          //  return __awaiter(this, void 0, void 0,pippo);pippo= async function* () {
           ...
           // **************   TODO :                >>>> PERFORMANCE ENHANCE substitute     is_prevCollKey=previous.collect.key; and in log : ',prevName,'
            ...
            let goonDir = true,// default : do goon(mixedinitiuative) tech
                ...
                logsout+='\n ***** PROCESSING previous  ONSTEP '+ prevName+ '  at current index: '+ step.index+ '  thread : '+ step.thread+ ' cmd : '+ dc.activeDialog.id;
...
//let a=this._vcontroller.init;
                ////// WARNING   is_prevCollKey = ' ' +  prevName    ?????????????????
                if(logsout){
                    if(is_prevCollKey)logsout+='\n  >> Matching  Previous Ask  '+ is_prevCollKey+ '  firing OnChange ';
                ...
                if(!step.result)msgSent=-1000;
                 ...
                 if(logsout){logsout+=', on OBJ RESULT , so result is not a user answer to current or previous step template but  coming from GOTOTHREAD/REPLY(result=null)/REPLACEDIALOG ( also returning from child)) is available. '+
            ...
            if(!step.msgSent >-2)console.error(' RREEDD  just WARNING : RELAXED goon because step.msgSent, >-2 meand the previus message comes from the ask that goon to this th or a this th ask : \n step.msgSent=',step.msgSent,' >-2 condition allows to work with child obj return matching continue routing. '); 
                ...
                if (line.collect && line.action !== 'beginDialog') {// 022021  2 alternative sendactivity o dc.prompt uno (dc.prompt) causa la fine del turno ??
                    try {
                        ...
                        return yield dc.prompt(this._prompt, this.makeOutgoing(line, step.values,step,dc));
                    }
                    catch (err) {
                        console.error(err);
